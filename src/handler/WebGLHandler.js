...
export default class WebGLHandler {
  constructor(canvas) {
    ...
    this._attributes = ['a_position', 'a_normal', 'a_color'];
  ...
  draw(props) {
    for (let i = 0; i < this._nodesDetails.objectNodes.length; i++) {
      this._updateProperties(props, this._nodesDetails.objectNodes[i]);
      this.drawModel(props, this._nodesDetails.objectNodes[i]);
    }

    return this;
  }

  drawModel(props, objectNode) {
    let glVertices = [];
    let glColors = [];
    let glNormals = [];

    this._initBuffers(glVertices, glColors, glNormals, objectNode.model);
